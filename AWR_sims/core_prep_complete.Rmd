...
This is data from NCACC.org from the link http://www.ncacc.org/DocumentCenter/View/3858 on the page http://www.ncacc.org/197/Election-Results (7/21/18). The file delivered is '2016 Election Demographics for Website_1.xlsx'. I expect this name to change after the 2018 election, since it is not consistent with the names of other files on that same page.  

(1/16/19) There has been a change. Now there is '2016 Election Demographics for Website_5.xlsx' as well as '2018 Election Demographics for Website_1.xlsx'  

This spreadsheet appears in not quite curated condition. I have built my data file 'NCACC_2002_2018_summary.csv' by hand, since the spreadsheets makes it inordinately difficult to do so programatically.  

```{r mosaic}
df_NCACC_summary <- read.csv(
  paste0(data_dir,"NCACC_2002_2018_summary.csv"),
  header=TRUE,
  sep="\t",
  stringsAsFactors = FALSE
) %>%
...
Change this as of 1/21/2019 to use updated xls files provided by NCACC  
and munged by DIH to properly deal with 0 items.  

Files are  
 "NCACC 2016 Election Demographics for Website_5.2.csv"  
 [1] "County"           "No..of.Comm."     "New"              "Repub."           "Dem."  
 [6] "Other"            "Female"           "White"            "African.American" "American.Indian"  
[11] "Other.1"  

 "NCACC 2018 Election Demographics for Website_1.2.csv"  
 [1] "County"          "No..of.Comm."    "New"             "Repub."          "Dem."  
 [6] "Other.Party"     "Female"          "White"           "Black"           "American.Indian"  
[11] "Other.Race"  


```{r  grown}
df_NCACC_2016_demog <- read.csv(
  paste0(data_dir,"NCACC 2016 Election Demographics for Website_5.2.csv"),
  sep="\t",
  header=TRUE,
  stringsAsFactors=FALSE
) %>%
...
#
df_NCACC_2018_demog <- read.csv(
  paste0(data_dir,"NCACC 2018 Election Demographics for Website_1.2.csv"),
  sep="\t",
  header=TRUE,
  stringsAsFactors=FALSE
) %>%
...
Column names are the same so we can bind dataframes.  
 [1] "County"          "No..of.Comm."    "New"             "Repub."          "Dem."  
 [6] "Other.Party"     "Female"          "White"           "Black"           "American.Indian"  
[11] "Other.Race"      "Year"  
...
df_NCACC_16_18 <-
  ...
  plyr::rename(c(
    ...
    "White"="nwhite",
    "Black"="nafam",
    "American.Indian"="namin",
    "Other.Race"="nother_race"
    )
  )
#
str(df_NCACC_16_18)
...
Names are now  
 [1] "County"      "ncomm"       "new"         "nrep"        "ndem"        "nother"      "nfemale"   
 [8] "nwhite"      "nafam"       "namin"       "nother_race" "Year"  
...
xls file formats changed in 2014 and again in 2016.  
Note some items are omitted when 0 but some 0s are there.  
2006 County	New	Repub.	Dem.	Other	No. of Comm.	Female	African American	Native American	Other  
2008 County	New	Repub.	Dem.	Other	No. of Comm.	Female	African American	Native American	Other  
2010 County	New	Repub.	Dem.	Other	No. of Comm.	Female	African American	Native American	Other  
2012 County	New	Repub.	Dem.	Other	No. of Comm.	Female	African American	Native American	Other  

2014 County	No. of Comm.	New	Repub.	Dem.	Other	Female	African American	Native American	Other  

2016 County	No. of Comm.	New	Repub.	Dem.	Other	Female	White	African American	American Indian	Other  
2018 County	No. of Comm.	New	Repub.	Dem.	Other Party	Female	White	Black	American Indian	Other Race  

1/21/2019 DIH munged the 2014 NCACC file by hand. Moved no. of commis column to match position in earlier NCACC files.  

```{r gear}
fn_NCACC_build <- function(){
  #
  NCACC_files <- list(
    c("2004","NCACC_electionresults04_201209251258330448.2.csv"),
    c("2006","NCACC_electionresults06_201209251258337011.2.csv"),
    c("2008","NCACC_electionresults08_201209251258339823.2.csv"),
    c("2010","NCACC_electionresults10_201209251258349042.2.csv"),
    c("2012","NCACC_electionresults12_201302190817370069.2.csv"),
    c("2014","NCACC_electionresults14_201503031245453010.3.csv")
  )
  for (i in 1:length(NCACC_files)){
    ...
    #
    if (!exists("df_all",inherits=FALSE)){
      df_all <- df_NCACC_16_18
    } else {
      df_all <- rbind(
        df_all,
        df_
...
if (do_write_csv==TRUE){
  ...
  readr::write_csv(
    paste0(output_dir,"df_NCACC_years.csv"),
    col_names = TRUE
  )
}
```  
