...
df_ncvhis_dups  if any dups in ncvhis  
...
if (exists("df_ncvhis_dups")) {
  ...
  readr::write_csv(
    df_ncvhis_dups,
    paste0("df_ncvhis_dups_",as_of,".csv.gz")
  )
} else {
  print("No duplicates.")
}
...
dplyr::mutate(
  reason_cd=ifelse(trimws(reason_cd)=="",NA,race_code),
  race_code=ifelse(trimws(race_code)=="","U",race_code),
  gender_code=ifelse(trimws(gender_code)=="","U",gender_code),
...
readr::write_csv(
  df_ncvoter_in,
  paste0("df_ncvoter_in_",as_of,".csv.gz")
)
```  
...
readr::write_csv(
  df_ncvhis_in,
  paste0("df_ncvhis_in_",as_of,".csv.gz")
)
```  
...
readr::write_csv(
  df_nc_voted,
  paste0("df_nc_voted_",as_of,".csv.gz")
)
```  
